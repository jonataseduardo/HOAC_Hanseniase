...
fit_pipeline <-
  ...
  if(nivel == 'macro'){
    groupby_nivel = c("no_regiao_brasil", "sg_uf", "no_macrorregiao")
    groupby = groupby_nivel
    num_cases  <-
      eval_num_cases(data,
                     col_usuario,
                     col_populacao,
                     groupby = c(groupby_nivel, 'nu_ano'))
    fit <- lmer(log(NCDR) ~ nu_ano + (1 + nu_ano || sg_uf / no_macrorregiao),
                data = num_cases[NCDR > 0])
    new_data <- make_newdata(num_cases, 50, groupby = groupby_nivel)
    pred_data <- fit_ci(fit, new_data, nivel)
  }
  if(nivel == 'uf'){
    groupby_nivel = c("no_regiao_brasil", "sg_uf" )
    groupby = groupby_nivel
    num_cases  <-
      eval_num_cases(data,
                     col_usuario,
...
anlz_pipeline <-
  ...
  if(grouping == 'subnotificacao'){
    fit_result_sub_lower <-
      fit_pipeline(data, level, 'qt_usuario_tx_deteccao_hpd90lower', 'qt_populacao', 'subnotificação inferior')
    fit_result_sub_media <-
      fit_pipeline(data, level, 'qt_usuario_tx_deteccao_mean', 'qt_populacao', 'subnotificação média')
    fit_result_sub_upper <-
      fit_pipeline(data, level, 'qt_usuario_tx_deteccao_hpd90upper', 'qt_populacao', 'subnotificação superior')

    fit_data <-
      rbindlist(list(fit_result_sub_lower[[1]], fit_result_sub_media[[1]], fit_result_sub_upper[[1]]))

    fit_coef <-
      rbindlist(list(fit_result_sub_lower[[2]], fit_result_sub_media[[2]], fit_result_sub_upper[[2]]))

    fit_model <-
     list(fit_result_sub_lower[[3]], fit_result_sub_media[[3]], fit_result_sub_upper[[3]])
  }

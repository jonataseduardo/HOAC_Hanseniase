...
num_cases <- 
  data[, .(NC = .N, LogNC = log(.N)), 
       keyby = .(nu_ano, sg_uf, co_macrorregiao, no_regiao_brasil)]

num_cases[, nu_ano := nu_ano - min(nu_ano)]


num_cases[, n_notific := .N, by = .(co_macrorregiao) ]
num_cases[n_notific < 19]


fit1 <- lmer(LogNC ~ nu_ano + (1 + nu_ano || sg_uf/co_macrorregiao ),
             data = num_cases,
             REML = FALSE
)
...
x <- coef_to_DF(fit1, 'co_macrorregiao')
x
